...
def plot_all_entropies(entropies: dict[str, Any], hidden_state: int | str = "last", framework: str = "quanto", allow_2bit: bool = False):
    if framework == "quanto":
        colors = {
            "int2": "red",
            "int4": "orange",
    ...
    fig, axs = plt.subplots(len(entropies), 4, figsize=(12, 12))

    for i, (model, _entropies) in enumerate(entropies.items()):
        ...
        for quantization, __entropies in _entropies.items():
            ...
            for j, metric in enumerate(["unnormalized_entropy", "logN_normalized_entropy", "logD_normalized_entropy", "logNlogD_normalized_entropy"]):
                if quantization_framework != framework:
                    continue
...
def plot_mean_perplexity(entropies: dict[str, Any], framework: str = "quanto"):
    model_sizes = [k.split("-")[-1] for k in entropies.keys()]
